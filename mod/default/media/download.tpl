...
{if $download.disabled != '1'}
	...
	{if $download.allowed == true}
		<form action="" method="post" name="dl">
			...
			<p>
				<input type="submit" name="downloadbutton" onClick="{$download.url} document.dl.submit('download');" value="&raquo; {$download.downloadthis} &laquo;" style="font-weight:bold; padding:10px;" />
			</p>
		</form>
	{else}
