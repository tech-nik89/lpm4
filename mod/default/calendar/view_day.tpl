	...
	{foreach item=hour from=$day_view}
		<tr class="{cycle values=',highlight_row'}">
			...
			{section name=i loop=$hour}
				{if $hour[i].rowspan!=0}
					{if $hour[i].title!=''}
						<td {if $hour[i].description != ''}onMouseMove="showTooltip('entry_{$hour[i].calandarid}');" onMouseOut="hideTooltip('entry_{$hour[i].calandarid}');" {/if} rowspan="{$hour[i].rowspan}" style="line-height:0px; overflow:hidden; vertical-align:top; {if $hour[i].title != ''} cursor:pointer; background-color: {$hour[i].backgroundcolor}; color: {$hour[i].fontcolor};<!--border:1px solid #19D144;!-->{/if}">	
...
{foreach item=hour from=$day_view}
	{section name=i loop=$hour}
		{if $hour[i].rowspan!=0}
			{if $hour[i].title!=''}
				<div id="entry_{$hour[i].calandarid}" style="max-width:300px; border:1pt solid black; padding: 5px; display:none; background-color: {$hour[i].backgroundcolor}; color: {$hour[i].fontcolor};">
					<strong>
						{$hour[i].start.hour}<sup>{$hour[i].start.min}</sup>
						-
						{$hour[i].end.hour}<sup>{$hour[i].end.min}</sup>
